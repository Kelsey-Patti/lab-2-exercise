...
struct Bullet : public sf::Drawable {
    ...
    void update() {
        ...
        shape.move(velocity);
        lifetime -= 1.0f / 60.0f;
        if (shape.getPosition().x <0 || shape.getPosition().x > WINDOW_WIDTH || shape.getPosition().y <0 ||shape.getPosition().y >WINDOW_HEIGHT || lifetime <=0.0f){
            isAlive = false;
        }
    }
...
public:
    ...
    void update(const InputSummary& inputSummary) {
        ...
        if (inputSummary.shootingDesired && SHOOT_COOLDOWN <= mShootClock.getElapsedTime().asSeconds()) {
            mBullets.emplace_back(mSpaceship.getPosition(), sf::Vector2f(cos(angleRadians), sin(angleRadians)) * BULLET_SPEED);
            mShootClock.restart();
        }
...
private:
    ...
    void processCollisionsSpaceshipAsteroid() {
        for (auto& asteroid : mAsteroids) {
            ...
            if (circlesIntersect(mSpaceship.getPosition(), SPACESHIP_HITBOX_RADIUS, asteroid.shape.getPosition(), ASTEROID_RADIUS)){
                mExplosionSound.play();
                asteroid.isAlive = false;
            }
        }
